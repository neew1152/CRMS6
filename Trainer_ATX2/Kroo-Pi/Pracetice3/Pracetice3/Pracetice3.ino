#include <POP32.h>
 ...
 //เดินแบบไม่ PID ใช้ percent
 #define val_S1 map(analog(4), min_S1, max_S1, 0, 100)
 #define val_S2 map(analog(0), min_S2, max_S2, 0, 100)
 #define val_L1 map(analog(1), min_S2, max_S2, 0, 100)
 #define val_R1 map(analog(3), min_S2, max_S2, 0, 100)
 #define val_BL2 map(analog(8), min_S2, max_S2, 0, 100)
 #define val_BR2 map(analog(5), min_S2, max_S2, 0, 100)
int min_S1 = 0;
int max_S1 = 1670;
int min_S2 = 0;
int max_S2 = 1670;
...
void setup() {
  ...
  oled.text(1,0," Medium 4 W33 POP32i");
  oled.text(2,0," T6NakornChiangrai ");
  ...
  while(SW_OK_Status == No)  {  // waiting "OK" button
      if(Read_sw_OK()) {
        ...
        if(ReadTimer() >= 500) {
          beep(2);delay(300);
          // Right(100,175);waitSW_OK();Left(100,190);
          Show7Analog();
          SW_OK_Status = No;
          }
        }
  ...
  oled.show();
  StartTimer2();
  zeroYaw(); //กำหนดมุมเริ่มต้นเป็น 0 องศา
