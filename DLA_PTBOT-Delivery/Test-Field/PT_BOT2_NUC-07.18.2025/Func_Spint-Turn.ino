void spinDegree(int relative_degree) {
  ...
  // KP too high + KD too low → shaking.
  // KD too high → response too slow.
  float kp = 0.8;                                 // ลด KP
  float kd = 1.0;                                 // เพิ่ม KD
  float small_angle_threshold = 15;               // หุ่นยนต์จะใช้ความเร็วต่ำสุดเมื่อเข้าใกล้องศาที่กำหนด
  float stop_threshold = 1.75;                     // กำหนดความคลาดเคลื่อนที่ยอมรับได้
