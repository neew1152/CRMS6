...
void FFobj(int obj_height) {
  if (obj_height != 0) FFUltrasonic(Ultrasonic_Pick_Distance);
  else FFToF(ToF_Distance);
  vTaskDelay(Delay_LOW / portTICK_PERIOD_MS);
}

void FFobj(int obj_height, char gripper) {
  switch (gripper) {
    case 'L':
...
void PickCan(int pick_height, int B_Speed, int B_delay) {
  PickCan(pick_height, 'A', B_Speed, B_delay);
}

void PickCan(int pick_height, char gripper, int B_Speed, int B_delay) {
  BalanceArm('O');
  Lift(pick_height);
  FFobj(pick_height, gripper);
  switch (gripper) {
    case 'L':
      servoWrite(Channel_Gripper_Left, Close_Left);
      break;
...
void DropCan(int Drop_height, char gripper, char Drop_point, int B_Speed, int B_delay) {
  Lift(Drop_height + 2);
