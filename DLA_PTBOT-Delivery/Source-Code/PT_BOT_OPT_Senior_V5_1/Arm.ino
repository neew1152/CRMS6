...
////////////////////////////////////////////////////////
int Swing_Left = 58;          //S0 สวิงแขนซ้าย เพิ่มง้างออก ลด หุบเข้า
int Swing_Right = 58;         //S1 สวิงแขนขวา เพิ่มง้างออก ลด หุบเข้า
...
////////////////////////////////////////////////////////
int Open_Left = 100;  //S2 อ้ามือ ชุดมือจับมือซ้ายค่ามากยิ่งหุบ
int Open_Right = 95;  //S3 อ้ามือ ชุดมือจับมือขวา ค่ามากยิ่งหุบ
////////////////////////////////////////////////////////
int Close_Left = 140;   //S2 หุบมือ ชุดมือจับมือซ้ายค่าน้อยยิ่งอ้า
int Close_Right = 135;  //S3 หุบมือ ชุดมือจับมือขวา ค่าน้อยยิ่งอ้า
////////////////////////////////////////////////////////
...
void setPlaceTime(float timeleft_side, float timeleft_center, float timeright_side, float timeright_center) {
  place_left_side = timeleft_side;        // ค่าระยะจอดกรณีมือซ้ายวางกระป๋องซ้าย
  place_left_center = timeleft_center;    // ค่าระยะจอดกรณีมือซ้ายวางกระป๋องกลาง
  place_right_side = timeright_side;      // ค่าระยะจอดกรณีมือขวาวางกระป๋องขวา
  place_right_center = timeright_center;  // ค่าระยะจอดกรณีมือขวาวางกระป๋องกลาง
}
...
void placeCan1_NoShake(int drop_height, char from_gripper, char drop_point, int B_speed, int B_delay) {
  // int drop_height = 0;
  // int drop_height1 = 0;
  ...
  // if (drop_height != 0) moveToCan(pick_can_distance);
  // else moveToCan0(can_distance);
  // lift(drop_height);
  // delay(400);
...
void placeCan1_NoShake_Plus(char color, char from_gripper, char drop_point) {
  int drop_height = 0;
  int drop_delay = 200;  // ค่าเริ่มต้นเผื่อไว้
  int UP_delay = 200;
...
void placeCan2_NoShake(char color, char left_gripper, char right_gripper, int B_speed, int B_delay) {
  int drop_height = 0;
  int drop_delay = 200;  // ค่าเริ่มต้นเผื่อไว้
  int UP_delay = 200;
...
void Balance_Arm(char select) {
  if (select == 'C') {
    servoWrite(chanal_Gripper_Left, Close_Left);
    servoWrite(chanal_Gripper_Right, Close_Right);
    servoWrite(chanal_Swing_Left, Swing_Left - 50);
    servoWrite(chanal_Swing_Right, Swing_Right - 50);
  } else if (select == 'O') {
    servoWrite(chanal_Swing_Left, Swing_Left);
    servoWrite(chanal_Swing_Right, Swing_Right);
    servoWrite(chanal_Gripper_Left, Open_Left);
...
void PLACE_LOCK_SONAR_ALL(int drop_height) {
  Balance_Arm('S');
  lift(drop_height);
  // lift(drop_height + 5);
...
void PLACE_LOCK_SONAR(int drop_height, char from_gripper) {
  Stable_CAN();
  lift(drop_height);
  // lift(drop_height + 5);
  delay(drop_height * 75);
  ...
  if (from_gripper == 'L') {
    servoWrite(chanal_Swing_Right, Swing_Right + 72);
    moveToCan(5);
    motorStop();
    servoWrite(chanal_Gripper_Left, Swing_Right);
    servoWrite(chanal_Gripper_Left, Close_Left - 10);
    delay(1000);
    servoWrite(chanal_Gripper_Left, Open_Left);
    delay(500);
  } else if (from_gripper == 'R') {
    servoWrite(chanal_Swing_Left, Swing_Left + 72);
    moveToCan(5);
    motorStop();
    servoWrite(chanal_Gripper_Right, Swing_Right);
    servoWrite(chanal_Gripper_Right, Close_Right - 10);
    delay(1000);
    servoWrite(chanal_Gripper_Right, Open_Right);
    delay(500);
...
void pickCan_NoShake_Enhanced(int pick_height, bool switch_lr, int B_speed, int B_delay) {
  Balance_Arm('O');
  lift(pick_height);
